...
device_event_deserialize(const unsigned char *buf, size_t len,
    ...
    if (buf[0] != DEVICE_EVENT_TYPE_CLIPBOARD) {
        LOGW("Unsupported device event type: %d", (int) buf[0]);
    ...
    switch (buf[0]) {
        case DEVICE_EVENT_TYPE_CLIPBOARD: {
            ...
            event->type = DEVICE_EVENT_TYPE_CLIPBOARD;
...
device_event_destroy(struct device_event *event) {
    if (event->type == DEVICE_EVENT_TYPE_CLIPBOARD) {
